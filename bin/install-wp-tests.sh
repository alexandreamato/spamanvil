#!/usr/bin/env bash
# Installs the WordPress test suite + a test database for PHPUnit integration tests.
# Usage: bin/install-wp-tests.sh <db-name> <db-user> <db-pass> [db-host] [wp-version] [skip-database-creation]
# Standard wp-cli scaffold script (https://github.com/wp-cli/scaffold-command).
...
download() {
	if command -v curl >/dev/null 2>&1; then
		...
		echo "Neither curl nor wget is available." && exit 1
	fi
}
