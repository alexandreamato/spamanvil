...
**Translations and `plugins_loaded` (1.18.1).** `SpamAnvil::init()` runs on `plugins_loaded`, but schema check and cron scheduling are deferred to `init` via `run_deferred_setup()`. Both reach `wp_schedule_event()`, which applies `cron_schedules`, where `add_cron_interval()` translates its label — and translating before `init` makes WordPress 6.7+ log "Translation loading was triggered too early". Keep any new `__()` out of the `plugins_loaded` path.
...
The activation redirect is driven by the `spamanvil_activation_redirect` transient, set by `spamanvil_activate()` in `spamanvil.php` since 1.1.1 (1.15.0 briefly set it a second time inside `SpamAnvil_Activator::activate()` too — removed in 1.18.1, because that method also runs on a DB-version change and would have redirected people out of nowhere after a schema bump). What 1.15.0 changed is the destination: unconfigured installs land on the wizard, configured ones keep `&welcome=1`.
